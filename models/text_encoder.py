...
class TextEncoder(nn.Module):
    def __init__(self, vocab_size=None, embed_dim=256, hidden_dim=512, encoder_type='transformer'):
        """
        参数说明：
          - vocab_size: 词表大小
        ...
        if encoder_type == 'lstm':
            ...
            # 使用预训练的 BERTTextEncoder, 此时不再需要 vocab_size，因为 BERT 需要有自己的词表
            self.encoder = BERTTextEncoder(embed_dim)
        else:
            raise ValueError("Unsupported encoder_type: choose 'lstm', 'transformer', or 'bert'")
